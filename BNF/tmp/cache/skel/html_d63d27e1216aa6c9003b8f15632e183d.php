 ...
 * Compile :   Fri, 06 Jan 2017 13:37:41 GMT
 * Boucles :   _art
 */ 
...
function BOUCLE_arthtml_d63d27e1216aa6c9003b8f15632e183d(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			array(
			array('=', 'articles.id_article', sql_quote(@$Pile[0]['id_article'], '', '')), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('articles.statut',sql_quote($in)) : 
...
// Temps de compilation total: 10.328 ms
//
