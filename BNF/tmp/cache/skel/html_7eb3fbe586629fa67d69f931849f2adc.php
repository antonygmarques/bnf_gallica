...
/*
 * Squelette : ../plugins-dist/mots/prive/objets/liste/mots-admin.html
 * Date :      Fri, 30 Sep 2016 21:38:56 GMT
 * Compile :   Fri, 06 Jan 2017 16:26:40 GMT
 * Boucles :   _liste_mot
 */ 

function BOUCLE_liste_mothtml_7eb3fbe586629fa67d69f931849f2adc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	if (!isset($command['table'])) {
		...
		$command['from'] = array('mots' => 'spip_mots','L1' => 'spip_mots_liens','L2' => 'spip_mots_liens');
		$command['type'] = array();
		$command['groupby'] = array("mots.id_mot");
		$command['join'] = array('L1' => array('mots','id_mot'), 'L2' => array('mots','id_mot'));
		$command['limit'] = '';
		$command['having'] = 
			array();
	}
	...
	$command['where'] = 
			...
			array('=', 'mots.id_mot', sql_quote(@$Pile[0]['id_mot'], '', '')))), (!(is_array(@$Pile[0]['id_groupe'])?count(@$Pile[0]['id_groupe']):strlen(@$Pile[0]['id_groupe'])) ? '' : ((is_array(@$Pile[0]['id_groupe'])) ? sql_in('mots.id_groupe',sql_quote($in1)) : 
			array('=', 'mots.id_groupe', sql_quote(@$Pile[0]['id_groupe'], '', '')))), (!(is_array(@$Pile[0]['id_rubrique'])?count(@$Pile[0]['id_rubrique']):strlen(@$Pile[0]['id_rubrique'])) ? '' : ((is_array(@$Pile[0]['id_rubrique'])) ? sql_in('L1.id_objet',sql_quote($in2)) : 
			array('=', 'L1.id_objet', sql_quote(@$Pile[0]['id_rubrique'], '', '')))), (!(is_array(@$Pile[0]['id_rubrique'])?count(@$Pile[0]['id_rubrique']):strlen(@$Pile[0]['id_rubrique'])) ? '' : 
			...
			array('=', 'L2.id_objet', sql_quote(@$Pile[0]['id_article'], '', '')))), (!(is_array(@$Pile[0]['id_article'])?count(@$Pile[0]['id_article']):strlen(@$Pile[0]['id_article'])) ? '' : 
			array('=', 'L2.objet', sql_quote('article'))), ((@$Pile[0]["where"]) ? (@$Pile[0]["where"]) : ''));
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/mots/prive/objets/liste/mots-admin.html','html_7eb3fbe586629fa67d69f931849f2adc','_liste_mot',5,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	...
	$l1 = _T('mots:info_supprimer_mot');$SP++;
	// RESULTATS
...
'
		' .
vide($Pile['vars'][$_zzz=(string)'url_voir'] = invalideur_session($Cache, (((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('voir', 'mot', invalideur_session($Cache, $Pile[$SP]['id_mot']))?" ":"") ? invalideur_session($Cache, parametre_url(generer_url_entite($Pile[$SP]['id_mot'],'mot'),'redirect',interdire_scripts(invalideur_session($Cache, entites_html(table_valeur(@$Pile[0], (string)'retour', null),true))))):''))) .
vide($Pile['vars'][$_zzz=(string)'url_edit'] = invalideur_session($Cache, (((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('modifier', 'mot', invalideur_session($Cache, $Pile[$SP]['id_mot']))?" ":"") ? invalideur_session($Cache, parametre_url(generer_url_ecrire('mot_edit',(	'id_mot=' .
			invalideur_session($Cache, $Pile[$SP]['id_mot']))),'redirect',interdire_scripts(invalideur_session($Cache, entites_html(table_valeur(@$Pile[0], (string)'retour', null),true))))):''))) .
'<tr class="' .
...
((!is_array($l = quete_logo('id_mot', 'ON', $Pile[$SP]['id_mot'],'', 0))) ? '':
 ("<img class=\"spip_logo spip_logos\" alt=\"\" src=\"$l[0]\"" . $l[2] .  ($l[1] ? " onmouseover=\"this.src='$l[1]'\" onmouseout=\"this.src='$l[0]'\"" : "") . ' />'))) ?'' :' ')))!=='' ?
		...
		'') .
'>' .
(($t1 = strval(table_valeur($Pile["vars"], (string)'url_voir', null)))!=='' ?
		('<a href="' . $t1 . '">') :
		'') .
...
((table_valeur($Pile["vars"], (string)'url_voir', null))  ?
		('</a>' . ' ') :
		'') .
'</td>
			' .
...
(!(table_valeur($Pile["vars"], (string)'utilise', null))  ?
		(' ' . (	'
					' .
	(($t2 = strval(invalideur_session($Cache, ((((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('supprimer', 'mot', invalideur_session($Cache, $Pile[$SP]['id_mot']))?" ":"")) ?' ' :''))))!=='' ?
			($t2 . (	'
						' .
		bouton_action($l1,invalideur_session($Cache, generer_action_auteur('supprimer_mot',invalideur_session($Cache, $Pile[$SP]['id_mot']),invalideur_session($Cache, self()))),'ajax','','','(function(me){$(me).parents("tr").animateRemove();return true;})(this)') .
		'
					')) :
			'') .
	'
...
'
			...
			<td class=\'id\'>' .
(($t1 = strval(table_valeur($Pile["vars"], (string)'url_edit', null)))!=='' ?
		('<a href="' . $t1 . '">') :
		'') .
$Pile[$SP]['id_mot'] .
((table_valeur($Pile["vars"], (string)'url_edit', null))  ?
		('</a>' . ' ') :
		'') .
'</td>
		</tr>
	...
	AND 1000*($timer = (time()+microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_liste_mot @ ../plugins-dist/mots/prive/objets/liste/mots-admin.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../plugins-dist/mots/prive/objets/liste/mots-admin.html
// Temps de compilation total: 15.836 ms
//

function html_7eb3fbe586629fa67d69f931849f2adc($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
	...
	$connect = '';
	$page = (
(($t1 = strval(vide($Pile['vars'][$_zzz=(string)'defaut_tri'] = array('multi titre' => '1', 'id_mot' => '1
'))))!=='' ?
		($t1 . '
') :
		'') .
(($t1 = BOUCLE_liste_mothtml_7eb3fbe586629fa67d69f931849f2adc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
' .
	...
	<thead>
		...
		lien_ou_expose(parametre_url(parametre_url(self(),(($s=in_array('multi titre',array('>','<')))?'sens':'tri').'_liste_mot',$s?(strpos('< >','multi titre')-1):'multi titre'),'var_memotri',strncmp('_liste_mot','session',7)==0?(($s=in_array('multi titre',array('>','<')))?'sens':'tri').'_liste_mot':''),_T('public|spip|ecrire:info_titre'),$s?(((intval($t=(isset($Pile[0]['sens'.'_liste_mot']))?$Pile[0]['sens'.'_liste_mot']:((strncmp('_liste_mot','session',7)==0 AND session_get('sens'.'_liste_mot'))?session_get('sens'.'_liste_mot'):(is_array($s=table_valeur($Pile["vars"], (string)'defaut_tri', null))?(isset($s[$st=(($t=(isset($Pile[0]['tri'.'_liste_mot']))?$Pile[0]['tri'.'_liste_mot']:((strncmp('_liste_mot','session',7)==0 AND session_get('tri'.'_liste_mot'))?session_get('tri'.'_liste_mot'):interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'num titre'),true))))?tri_protege_champ($t):'')])?$s[$st]:reset($s)):$s)))==-1 OR $t=='inverse')?-1:1)==(strpos('< >','multi titre')-1)):((($t=(isset($Pile[0]['tri'.'_liste_mot']))?$Pile[0]['tri'.'_liste_mot']:((strncmp('_liste_mot','session',7)==0 AND session_get('tri'.'_liste_mot'))?session_get('tri'.'_liste_mot'):interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'num titre'),true))))?tri_protege_champ($t):'')=='multi titre'),'ajax') .
		'</th>
...
');
	...
	return analyse_resultat_skel('html_7eb3fbe586629fa67d69f931849f2adc', $Cache, $page, '../plugins-dist/mots/prive/objets/liste/mots-admin.html');
}
?>